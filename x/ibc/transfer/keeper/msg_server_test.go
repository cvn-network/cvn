	...
	testCases := []struct {
	...
	}{
		{
			...
			func() *types.MsgTransfer {
				...
				transferMsg := types.NewMsgTransfer("transfer", "channel-0", sdk.NewCoin("acvnt", sdk.NewInt(10)), senderAcc.String(), "", timeoutHeight, 0, "")

				coins := sdk.NewCoins(sdk.NewCoin("acvnt", sdk.NewInt(10)))
		...
		},
		{
			...
			func() *types.MsgTransfer {
				...
				suite.Require().NoError(err)
				suite.Commit()
				suite.Require().Equal("atokent", pair.Denom)
