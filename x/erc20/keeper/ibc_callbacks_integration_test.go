...
var _ = Describe("Convert receiving IBC to Erc20", Ordered, func() {
	...
	Describe("disabled params", func() {
		BeforeEach(func() {
			...
			err := s.app.Erc20Keeper.SetParams(s.CVNChain.GetContext(), erc20params)
			s.Require().NoError(err)

			sender = s.IBCOsmosisChain.SenderAccount.GetAddress().String()
			receiver = s.CVNChain.SenderAccount.GetAddress().String()
			receiverAcc = sdk.MustAccAddressFromBech32(receiver)
		})
		It("should transfer and not convert to erc20", func() {
			// register the pair to check that it was not converted to ERC-20
			pair, err := s.app.Erc20Keeper.RegisterCoin(s.CVNChain.GetContext(), osmoMeta)
			s.Require().NoError(err)

			// check balance before transfer is 0
			ibcOsmoBalanceBefore := s.app.BankKeeper.GetBalance(s.CVNChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
			...
			ibcOsmoBalanceAfter := s.app.BankKeeper.GetBalance(s.CVNChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
			s.Require().Equal(amount, ibcOsmoBalanceAfter.Amount.Int64())

			// check ERC20 balance - should be zero (no conversion)
			balanceERC20TokenAfter := s.app.Erc20Keeper.BalanceOf(s.CVNChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(receiverAcc.Bytes()))
	...
	Describe("enabled params and registered uosmo", func() {
		...
		It("should transfer and convert uosmo to tokens", func() {
			...
			balanceTokenBefore := s.app.Erc20Keeper.BalanceOf(s.CVNChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(receiverAcc.Bytes()))
			s.Require().Equal(int64(0), balanceTokenBefore.Int64())

			ibcOsmoBalanceBefore := s.app.BankKeeper.GetBalance(s.CVNChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
			s.Require().Equal(int64(0), ibcOsmoBalanceBefore.Amount.Int64())

			s.CVNChain.Coordinator.CommitBlock()
			// Send coins
			s.SendAndReceiveMessage(s.pathOsmosisEvmos, s.IBCOsmosisChain, "uosmo", amount, sender, receiver, 1, "")

			// Check ERC20 balances
			balanceTokenAfter := s.app.Erc20Keeper.BalanceOf(s.CVNChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(receiverAcc.Bytes()))
		...
		It("should transfer and not convert unregistered coin (uatom)", func() {
			...
			// check balance before transfer is 0
			ibcAtomBalanceBefore := s.app.BankKeeper.GetBalance(s.CVNChain.GetContext(), receiverAcc, teststypes.UatomIbcdenom)
			...
			// check balance after transfer
			ibcAtomBalanceAfter := s.app.BankKeeper.GetBalance(s.CVNChain.GetContext(), receiverAcc, teststypes.UatomIbcdenom)
		...
		It("should transfer and not convert acvnt", func() {
			...
			// 1. Send acvnt from Evmos to Osmosis
			s.SendAndReceiveMessage(s.pathOsmosisEvmos, s.CVNChain, utils.BaseDenom, amount, receiver, sender, 1, "")

			aevmosAfterBalance := s.app.BankKeeper.GetBalance(s.CVNChain.GetContext(), receiverAcc, utils.BaseDenom)
			s.Require().Equal(aevmosInitialBalance.Amount.Sub(math.NewInt(amount)).Sub(sendAndReceiveMsgFee), aevmosAfterBalance.Amount)

			// check ibc acvnt coins balance on Osmosis
			aevmosIBCBalanceBefore := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), senderAcc, teststypes.AcvntIbcdenom)
			s.Require().Equal(amount, aevmosIBCBalanceBefore.Amount.Int64())

			// 2. Send acvnt IBC coins from Osmosis to Evmos
			ibcCoinMeta := fmt.Sprintf("%s/%s", teststypes.AcvntDenomtrace.Path, teststypes.AcvntDenomtrace.BaseDenom)
			s.SendBackCoins(s.pathOsmosisEvmos, s.IBCOsmosisChain, teststypes.AcvntIbcdenom, amount, sender, receiver, 1, ibcCoinMeta)

			// check ibc acvnt coins balance on Osmosis - should be zero
			aevmosIBCSenderFinalBalance := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), senderAcc, teststypes.AcvntIbcdenom)
			s.Require().Equal(int64(0), aevmosIBCSenderFinalBalance.Amount.Int64())

			// check acvnt balance after transfer - should be equal to initial balance
			aevmosFinalBalance := s.app.BankKeeper.GetBalance(s.CVNChain.GetContext(), receiverAcc, utils.BaseDenom)
			...
			ibcCoinsBalance := s.app.BankKeeper.GetBalance(s.CVNChain.GetContext(), receiverAcc, teststypes.AcvntIbcdenom)
		...
		It("should transfer and convert original erc20", func() {
			...
			balanceERC20Token := s.app.Erc20Keeper.BalanceOf(s.CVNChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(receiverAcc.Bytes()))
			...
			s.SendBackCoins(s.pathOsmosisEvmos, s.CVNChain, types.ModuleName+"/"+pair.GetERC20Contract().String(), amount, receiver, sender, 1, ibcCoinMeta)
			...
			// check IBC Coin balance - should be zero
			ibcCoinsBalance := s.app.BankKeeper.GetBalance(s.CVNChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
			s.Require().Equal(int64(0), ibcCoinsBalance.Amount.Int64())
	...
	Describe("Performing recovery with registered coin", func() {
		...
		It("should recover and not convert uosmo to tokens", func() {
			...
			s.SendAndReceiveMessage(s.pathOsmosisEvmos, s.IBCOsmosisChain, "uosmo", amount, sender, receiver, 1, "")
			timeout := uint64(s.CVNChain.GetContext().BlockTime().Add(time.Hour * 4).Add(time.Second * -20).UnixNano())
	...
	Describe("Performing claims with registered coin", func() {
		BeforeEach(func() {
			s.app.Erc20Keeper.SetParams(s.CVNChain.GetContext(), types.DefaultParams()) //nolint:errcheck
			...
			params := s.app.ClaimsKeeper.GetParams(s.CVNChain.GetContext())
			params.AuthorizedChannels = []string{
				"channel-0",
			}
			s.app.ClaimsKeeper.SetParams(s.CVNChain.GetContext(), params) //nolint:errcheck
		})
		It("it should perform the claim and convert the received tokens", func() {
			...
			s.app.ClaimsKeeper.SetClaimsRecord(s.CVNChain.GetContext(), senderAcc, claimstypes.ClaimsRecord{
			...
			err := testutil.FundModuleAccount(s.CVNChain.GetContext(), s.app.BankKeeper, claimstypes.ModuleName, coins)
			s.Require().NoError(err)

			receiverInitialAevmosBalance := s.app.BankKeeper.GetBalance(s.CVNChain.GetContext(), receiverAcc, utils.BaseDenom)
			...
			// should trigger claims logic and send acvnt coins from claims to receiver
			...
			receiverFinalAevmosBalance := s.app.BankKeeper.GetBalance(s.CVNChain.GetContext(), receiverAcc, utils.BaseDenom)
	...
	Describe("registered erc20", func() {
		BeforeEach(func() { //nolint:dupl
			...
			sender = s.CVNChain.SenderAccount.GetAddress().String()
			receiverAcc = sdk.MustAccAddressFromBech32(receiver)
			senderAcc = sdk.MustAccAddressFromBech32(sender)
			...
			s.Require().NoError(err)
			pair, err = s.app.Erc20Keeper.RegisterERC20(s.CVNChain.GetContext(), addr)
			s.Require().NoError(err)
			...
			s.CVNChain.SenderAccount.SetSequence(s.CVNChain.SenderAccount.GetSequence() + 1) //nolint:errcheck
		})
		It("should convert erc20 ibc voucher to original erc20", func() {
			// Mint tokens and send to receiver
			_, err := s.app.Erc20Keeper.CallEVM(s.CVNChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, common.BytesToAddress(senderAcc.Bytes()), pair.GetERC20Contract(), true, "mint", common.BytesToAddress(senderAcc.Bytes()), big.NewInt(amount))
			s.Require().NoError(err)
			// Check Balance
			balanceToken := s.app.Erc20Keeper.BalanceOf(s.CVNChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			...
			_, err = s.app.Erc20Keeper.ConvertERC20(sdk.WrapSDKContext(s.CVNChain.GetContext()), msgConvertERC20)
			s.Require().NoError(err)
			...
			erc20CoinsBalance := s.app.BankKeeper.GetBalance(s.CVNChain.GetContext(), senderAcc, pair.Denom)
			s.Require().Equal(amount, erc20CoinsBalance.Amount.Int64())

			s.CVNChain.Coordinator.CommitBlock()

			// Attempt to send erc20 into ibc, should send without conversion
			s.SendBackCoins(s.pathOsmosisEvmos, s.CVNChain, pair.Denom, amount, sender, receiver, 1, pair.Denom)
		...
		It("should convert full available balance of erc20 coin to original erc20 token", func() {
			...
			// Attempt to send erc20 into ibc, should send without conversion
			s.SendBackCoins(s.pathOsmosisEvmos, s.CVNChain, pair.Denom, amount/2, sender, receiver, 1, pair.Denom)
			s.IBCOsmosisChain.Coordinator.CommitBlock()
		...
		It("send native ERC-20 to osmosis, when sending back IBC coins should convert full balance back to erc20 token", func() {
			...
			s.SendBackCoins(s.pathOsmosisEvmos, s.CVNChain, types.ModuleName+"/"+pair.GetERC20Contract().String(), amount/2, sender, receiver, 1, "")
			s.IBCOsmosisChain.Coordinator.CommitBlock()
...
var _ = Describe("Convert outgoing ERC20 to IBC", Ordered, func() {
	...
	Describe("disabled params", func() {
		...
		It("should fail transfer and not convert to IBC", func() {
			...
			denom := originChain.App.(*app.CVN).StakingKeeper.BondDenom(originChain.GetContext())
	...
	Describe("registered erc20", func() {
		...
		It("should transfer available balance", func() {
			// Mint tokens and send to receiver
			_, err := s.app.Erc20Keeper.CallEVM(s.CVNChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, common.BytesToAddress(senderAcc.Bytes()), pair.GetERC20Contract(), true, "mint", common.BytesToAddress(senderAcc.Bytes()), big.NewInt(amount*2))
			s.Require().NoError(err)
			// Check Balance
			balanceToken := s.app.Erc20Keeper.BalanceOf(s.CVNChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			s.Require().Equal(amount*2, balanceToken.Int64())
	...
	Describe("registered coin", func() {
		BeforeEach(func() {
			...
			s.CVNChain.Coordinator.CommitBlock(s.CVNChain)
			erc20params.EnableErc20 = true
			err = s.app.Erc20Keeper.SetParams(s.CVNChain.GetContext(), erc20params)
			s.Require().NoError(err)
		...
		It("should convert erc20 to ibc vouched and transfer", func() {
			...
			balance := s.app.BankKeeper.GetBalance(s.CVNChain.GetContext(), senderAcc, teststypes.UosmoIbcdenom)
			...
			s.Require().NoError(err)
			// Use MsgConvertERC20 to convert the ERC20 to a Cosmos IBC Coin
			_, err = s.app.Erc20Keeper.ConvertCoin(sdk.WrapSDKContext(s.CVNChain.GetContext()), msgConvertCoin)
			s.Require().NoError(err)

			s.CVNChain.Coordinator.CommitBlock()

			// Attempt to send erc20 tokens to osmosis and convert automatically
			s.SendBackCoins(s.pathOsmosisEvmos, s.CVNChain, pair.Denom, amount, sender, receiver, 1, teststypes.UosmoDenomtrace.GetFullDenomPath())
			s.IBCOsmosisChain.Coordinator.CommitBlock()
			// Check balance on the Osmosis chain
		...
		It("should timeout and reconvert coins", func() {
			...
			originChain := s.CVNChain
			coin := pair.Denom
			currentTime := s.CVNChain.Coordinator.CurrentTime
			...
			balanceERC20TokenAfter := s.app.Erc20Keeper.BalanceOf(s.CVNChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, pair.GetERC20Contract(), common.BytesToAddress(senderAcc.Bytes()))
			s.Require().Equal(int64(0), balanceERC20TokenAfter.Int64())
