...
var _ = Describe("Recovery: Performing an IBC Transfer", Ordered, func() {
	coinEvmos := sdk.NewCoin("acvnt", sdk.NewInt(10000))
	...
	Describe("from a non-authorized chain", func() {
		BeforeEach(func() {
			...
			err := s.CVNChain.App.(*app.CVN).ClaimsKeeper.SetParams(s.CVNChain.GetContext(), params)
		...
		It("should transfer and not recover tokens", func() {
			...
			nativeEvmos := s.CVNChain.App.(*app.CVN).BankKeeper.GetBalance(s.CVNChain.GetContext(), senderAcc, "acvnt")
			Expect(nativeEvmos).To(Equal(coinEvmos))
			ibcOsmo := s.CVNChain.App.(*app.CVN).BankKeeper.GetBalance(s.CVNChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
		...
		Describe("to the sender's own eth_secp256k1 account on Evmos (sender == recipient)", func() {
			...
			Context("with disabled recovery parameter", func() {
				BeforeEach(func() {
					...
					s.CVNChain.App.(*app.CVN).RecoveryKeeper.SetParams(s.CVNChain.GetContext(), params) //nolint:errcheck
				})
			...
			Context("with a sender's claims record", func() {
				Context("without completed actions", func() {
					BeforeEach(func() {
						...
						s.CVNChain.App.(*app.CVN).ClaimsKeeper.SetClaimsRecord(s.CVNChain.GetContext(), senderAcc, claim)

						// update the escrowed account balance to maintain the invariant
						err := testutil.FundModuleAccount(s.CVNChain.GetContext(), s.CVNChain.App.(*app.CVN).BankKeeper, claimstypes.ModuleName, coins)
						s.Require().NoError(err)
					})
				...
				Context("with completed actions", func() {
					...
					BeforeEach(func() {
						amt := sdk.NewInt(int64(100))
						coins := sdk.NewCoins(sdk.NewCoin("acvnt", sdk.NewInt(int64(75))))
						claim = claimstypes.NewClaimsRecord(amt)
						...
						// acvnt & ibc tokens that originated from the sender's chain
						s.SendAndReceiveMessage(s.pathOsmosisEvmos, s.IBCOsmosisChain, coinOsmo.Denom, coinOsmo.Amount.Int64(), sender, receiver, 1)
					...
					It("should transfer tokens to the recipient and perform recovery", func() {
						...
						balanceEscrow := s.CVNChain.App.(*app.CVN).BankKeeper.GetBalance(s.CVNChain.GetContext(), transfertypes.GetEscrowAddress("transfer", "channel-0"), "acvnt")
						Expect(balanceEscrow).To(Equal(coinEvmos))
						ibcOsmo := s.CVNChain.App.(*app.CVN).BankKeeper.GetBalance(s.CVNChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
						Expect(ibcOsmo.IsZero()).To(BeTrue())

						// Relay both packets that were sent in the ibc_callback
						err := s.pathOsmosisEvmos.RelayPacket(CreatePacket("10000", "acvnt", sender, receiver, "transfer", "channel-0", "transfer", "channel-0", 1, timeout))
						s.Require().NoError(err)
						...
						// Check that the acvnt were recovered
						nativeEvmos := s.CVNChain.App.(*app.CVN).BankKeeper.GetBalance(s.CVNChain.GetContext(), senderAcc, "acvnt")
						Expect(nativeEvmos.IsZero()).To(BeTrue())
						ibcEvmos := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), receiverAcc, teststypes.AcvntIbcdenom)
						Expect(ibcEvmos).To(Equal(sdk.NewCoin(teststypes.AcvntIbcdenom, coinEvmos.Amount)))
					...
					It("should not claim/migrate/merge claims records", func() {
						...
						claimAfter, _ := s.CVNChain.App.(*app.CVN).ClaimsKeeper.GetClaimsRecord(s.CVNChain.GetContext(), senderAcc)
			...
			Context("without a sender's claims record", func() {
				...
				When("recipient has additional ibc vouchers that originated from other chains", func() {
					...
					It("should not recover tokens that originated from other chains", func() {
						...
						// Acvnt was recovered from user address
						nativeEvmos := s.CVNChain.App.(*app.CVN).BankKeeper.GetBalance(s.CVNChain.GetContext(), senderAcc, "acvnt")
						Expect(nativeEvmos.IsZero()).To(BeTrue())
						ibcEvmos := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), receiverAcc, teststypes.AcvntIbcdenom)
						Expect(ibcEvmos).To(Equal(sdk.NewCoin(teststypes.AcvntIbcdenom, coinEvmos.Amount)))
						...
						ibcAtom := s.CVNChain.App.(*app.CVN).BankKeeper.GetBalance(s.CVNChain.GetContext(), senderAcc, teststypes.UatomIbcdenom)
				...
				When("recipient has additional non-native ibc vouchers that originated from senders chains", func() {
					BeforeEach(func() {
						params := types.DefaultParams()
						params.EnableRecovery = false
						err := s.CVNChain.App.(*app.CVN).RecoveryKeeper.SetParams(s.CVNChain.GetContext(), params)
						s.Require().NoError(err)
						...
						osmoIBCAtom := s.CVNChain.App.(*app.CVN).BankKeeper.GetBalance(s.CVNChain.GetContext(), receiverAcc, teststypes.UatomOsmoIbcdenom)
