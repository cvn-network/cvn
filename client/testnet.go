...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		coins := sdk.Coins{
			sdk.NewCoin(cvntypes.AttoCvnt, accStakingTokens),
		}
		...
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(cvntypes.AttoCvnt, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(cvntypes.AttoCvnt)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, cvntypes.AttoCvnt, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
