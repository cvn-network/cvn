...
init_func() {
    "$PWD"/build/cvnd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/cvnd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/cvnd add-genesis-account \
    "$("$PWD"/build/cvnd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000acvnt \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/cvnd gentx "$KEY$i" 1000000000000000000acvnt --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/cvnd collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/cvnd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/cvnd start --pruning=nothing --rpc.unsafe \
    ...
    CVN_PID=$!
    echo "started cvn node, pid=$CVN_PID"
    # add PID to array
    arr+=("$CVN_PID")
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test cvn node $HOST_RPC ..."
...
stop_func() {
    CVN_PID=$i
    echo "shutting down node, pid=$CVN_PID ..."
